...
@observer
export class CalibrationProgress extends React.Component<MyProps, {}> {
    ...
    @computed get statusMessages() {
		let process =  this.props.rsSimulation.progressMessage.filter( pm => pm.type == "run_recalibration");
		if (process.length) {
			return process;
		} else {
			return [{
				type: "run_recalibration",
				label: "Starting...",
				currentMessage: "",
