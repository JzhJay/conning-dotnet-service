...
@observer
export class ObservableTree extends React.Component<ITreeProps, {}> {
    public static nodeFromPath(path: number[], treeNodes: ITreeNode[]): ITreeNode {
        if (path.length === 1) {
            return treeNodes[path[0]];
        } else {
            return ObservableTree.nodeFromPath(path.slice(1), treeNodes[path[0]].childNodes);
        }
    }
    ...
    private handlerHelper(handlerFromProps: TreeEventHandler, node: TreeNode, e: React.MouseEvent<HTMLElement>) {
        if (isFunction(handlerFromProps)) {
            const nodeData = ObservableTree.nodeFromPath(node.props.path, this.props.contents);
