...
export class ChartGestures {
    ...
    private _startFirstPt: wijmo.Point = null;
    ...
    private _pointerMove(e: any) {
        var pt1, pt2,
            mvPt = new wijmo.Point(e.pageX, e.pageY),
    ...
    private _onTouchMove(e: any) {
        ...
        var scale = {}, offset = {},
            touchs = e.touches[0],
            mvPt = new wijmo.Point(touchs.pageX, touchs.pageY),
            rNowCordinate, rStartCordinate,
    ...
    private _updateOverLay(mvPt: wijmo.Point) {
    ...
    _pointInPlotArea(mvPt: wijmo.Point) {
        var plotRect = this._plotOffset;
    ...
    private _zoomedChart(endPt: wijmo.Point) {
        if (!endPt) {
            return;
        }
        //horizontal
    ...
    private _zoomedAxis(endPt: wijmo.Point, isX: boolean) {
    ...
    private _panningChart(distanceX: number, distanceY: number) {
        ...
        let dx = axisX.reversed ? -distanceX : distanceX;
        // check x axis range
        if (dx > 0 && axisX.actualMin.valueOf() === this._minX) {
            distanceX = 0;
        }
        if (dx < 0 && axisX.actualMax.valueOf() === this._maxX) {
            distanceX = 0;
        }

        let dy = axisY.reversed ? -distanceY : distanceY;
        // check y axis range
        if (dy > 0 && axisY.actualMax.valueOf() === this._maxY) {
            distanceY = 0;
        }
        if (dy < 0 && axisY.actualMin.valueOf() === this._minY) {
            distanceY = 0;
        }
    ...
    private _updateAxisByDistance(isX: boolean, distance: number) {
        ...
        let d = axis.reversed ? -distance : distance;
    ...
    private _updateAxisRange(axis: wijmo.chart.Axis, tMin: number, tMax: number) {
    ...
    private _getAxisMin(axis: wijmo.chart.Axis) {
        return wijmo.isDate(axis.actualMin) ? axis.actualMin.valueOf() : axis.actualMin;
    }

    private _getAxisMax(axis: wijmo.chart.Axis) {
    ...
    private _getPoint(e: any): wijmo.Point {
...
export class _RangeSlider {
    ...
    private _movingOffset: wijmo.Rect = null;
    private _range: number = null;
    private _plotBox;
    private _startPt: wijmo.Point = null;
    ...
    */
    readonly rangeChanged = new wijmo.Event<_RangeSlider, wijmo.EventArgs>();
    ...
    */
    readonly rangeChanging = new wijmo.Event<_RangeSlider, wijmo.EventArgs>();

    /**
     * Raises the {@link rangeChanging} event.
     */
    onRangeChanging(e?: wijmo.EventArgs) {
    ...
    private _createSlider(container: HTMLElement) {
        ...
        if (this._hasButtons) {
                //decrease button
            this._decBtn = wijmo.createElement(
            ...
            //increase button
            this._incBtn = wijmo.createElement(
    ...
    private _invalidate() {
        ...
        addCls = this._isHorizontal ?
        _RangeSlider._HRANGESLIDER : _RangeSlider._VRANGESLIDER;
        rmvCls = this._isHorizontal?
        _RangeSlider._VRANGESLIDER : _RangeSlider._HRANGESLIDER;

        wijmo.removeClass(this._rsEle, rmvCls);
        wijmo.addClass(this._rsEle, addCls);
    ...
    private _doSliding(offset, pt?: wijmo.Point) {
    ...
    private _setSlidingInterval(offset, pt?: wijmo.Point) {
        var self = this,
            sOffset, rOffset;
...
export class RangeSelector {
     ...
     */
    constructor(chart: wijmo.chart.FlexChartCore, options?) {
        this._chart = wijmo.asType(chart, wijmo.chart.FlexChartCore, false);
     ...
     * Occurs after the range changes.
     */
    rangeChanged = new wijmo.Event<RangeSelector, wijmo.EventArgs>();
    /**
     * Raises the {@link rangeChanged} event.
     */
    onRangeChanged(e?: wijmo.EventArgs) {
    ...
    private _adjustMinAndMax() {
        var self = this, chart = self._chart, rangeSlider = self._rangeSlider,
            ...
            actualMin = wijmo.isDate(axis.actualMin) ? axis.actualMin.valueOf() : axis.actualMin,
            actualMax = wijmo.isDate(axis.actualMax) ? axis.actualMax.valueOf() : axis.actualMax,
        ...
        if (plotRect) {
            ...
            if (this._orientation === Orientation.X) {
                minPos = (axis.convert(self._min) - plotRect.left) / plotRect.width;
                maxPos = (axis.convert(self._max) - plotRect.left) / plotRect.width;
            } else {
                minPos = (plotRect.top - axis.convert(self._min)) / plotRect.height + 1;
                maxPos = (plotRect.top - axis.convert(self._max)) / plotRect.height + 1;
            }
            rangeSlider._minPos = isNaN(minPos) ? 0 : minPos;
            rangeSlider._maxPos = isNaN(maxPos) ? 1 : maxPos;
...
wijmo._registerModule('wijmo.chart.interaction', wijmo.chart.interaction);
